...
public class UnicodeFontRenderer {
    ...
    private UnicodeFontRenderer(final String fontName, final int fontType, final float fontSize, final float kerning, final float antiAliasingFactor) {
        ...
        this.cachedStringWidth = (Map<String, Float>)new HashMap();
    ...
    private Font getFontByName(final String name) throws IOException, FontFormatException {
        return this.getFontFromInput(new StringBuilder("/assets/minecraft/clientname/fonts/").append(name).append(".ttf").toString());
    }
    ...
    public int drawString(final String text, float x, float y, final int color) {
        ...
        for (int length = (array = characters).length, i = 0; i < length; ++i) {
            ...
            if (c != '§' && (index == 0 || index == characters.length - 1 || characters[index - 1] != '§')) {
                ...
                final int codeIndex = "0123456789abcdefg".indexOf((int)text.charAt(index + 1));
                if (codeIndex < 0) {
                    continue;
                }
    ...
    public float getWidth(final String s) {
        ...
        return (float)this.cachedStringWidth.computeIfAbsent(s, (Function)new Function<String, Float>() {
            public Float apply(final String e) {
                ...
                for (int length = (charArray = str.toCharArray()).length, i = 0; i < length; ++i) {
                    ...
                    width += UnicodeFontRenderer.this.unicodeFont.getWidth(Character.toString(c)) + UnicodeFontRenderer.this.kerning;
                }
                return width / 2.0f / UnicodeFontRenderer.this.antiAliasingFactor;
            }
        });
    }
    ...
    public List<String> splitString(final String text, final int wrapWidth) {
        ...
        for (int length = (array = splitText).length, i = 0; i < length; ++i) {
            ...
            final String potential = new StringBuilder().append(currentString).append(" ").append(word).toString();
